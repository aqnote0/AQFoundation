...
+ (NSString *)mainBundleFileContent:(NSString *)fileName
  ...
  NSString *ret = [NSString stringWithContentsOfFile:filePath
                                            encoding:NSUTF8StringEncoding
                                               error:nil];
  if (!ret) {
    AQ_DEBUG(@"%s %@ missing content: %@", __FUNCTION__, @"", fileName);
  }
  return ret;
}
...
+ (NSString *)bundleFileContent:(NSString *)fileName
  ...
  NSURL *url = [AQBundle bundleFileURL:fileName
                                    fileType:fileType
  ...
  NSString *content = [NSString stringWithContentsOfURL:url
                                               encoding:NSUTF8StringEncoding
                                                  error:nil];
  if (!content) {
    AQ_DEBUG(@"%s missing content: %@.%@", __FUNCTION__, fileName, fileType);
  }
  return content;
}
...
+ (NSURL *)bundleFileURL:(NSString *)fileName
  ...
  if (![[NSFileManager defaultManager] fileExistsAtPath:filePath]) {
    AQ_ERROR(@"%s %@.bundle/%@.%@ is no exst.", __FUNCTION__, bundleName,
               fileName, fileType)
    return nil;
  }
