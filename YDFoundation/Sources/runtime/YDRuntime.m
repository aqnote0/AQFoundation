...
+ (NSArray *)rt_subclasses:(Class)parentClass {
  ...
  NSArray *classes = [YDRuntime rt_allclasses];
...
+ (NSArray *)rt_allClassesImplOf:(Protocol *)protocol {
  ...
  for ( Class classType in [YDRuntime rt_allclasses] ) {
...
+ (YDRTClass *)rt_createUnregisteredSubclass:(NSString *)name
  ...
  return
      [YDRTClass unregisteredClassWithName:name withSuperclass:parentClass];
}

+ (Class)rt_createSubclass:(NSString *)name parentClass:(Class)parentClass {
  return
      [[YDRuntime rt_createUnregisteredSubclass:name
...
+ (NSArray *)rt_methods:(Class)clazz {
  ...
  NSMutableArray *array = [NSMutableArray array];
  for (unsigned i = 0; i < count; i++)
    [array addObject:[YDRTMethod methodWithObjCMethod:methods[i]]];
...
+ (YDRTMethod *)rt_methodForSelector:(Class)clazz sel:(SEL)sel {
  ...
  return [YDRTMethod methodWithObjCMethod:method];
}
...
+ (NSArray *)rt_ivars:(Class)clazz {
  ...
  NSMutableArray *array = [NSMutableArray array];
  for (unsigned i = 0; i < count; i++)
    [array addObject:[YDRTIvar ivarWithObjCIvar:list[i]]];
...
+ (YDRTIvar *)rt_ivarForName:(Class)clazz varName:(NSString *)name {
  ...
  if (!ivar) return nil;
  return [YDRTIvar ivarWithObjCIvar:ivar];
}
...
+ (NSArray *)rt_properties:(Class)clazz {
  ...
  NSMutableArray *array = [NSMutableArray array];
  for (unsigned i = 0; i < count; i++)
    [array addObject:[YDRTProperty propertyWithObjCProperty:list[i]]];
...
+ (YDRTProperty *)rt_propertyForName:(Class)clazz propName:(NSString *)name {
  ...
  return [YDRTProperty propertyWithObjCProperty:property];
}
...
+ (Class)rt_propertyClassForName:(Class)clazz propName:(NSString *)name {
  ...
  YDRTProperty *rtProperty =
      [YDRTProperty propertyWithObjCProperty:property];
...
+ (id)rt_returnValue:(id)obj sel:(SEL)sel, ... {
  YDRTMethod *method =
      [YDRuntime rt_methodForSelector:object_getClass(obj) sel:sel];
  NSParameterAssert([[method signature]
      hasPrefix:[NSString stringWithUTF8String:@encode(id)]]);
  id retVal = nil;
  va_list args;
