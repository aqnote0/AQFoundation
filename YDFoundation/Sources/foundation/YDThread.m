...
+ (void)foreground:(dispatch_block_t)block {
  return [[YDThread sharedInstance] foreground:block];
}
...
+ (void)backgroundInitMain:(dispatch_block_t)block {
  return [[YDThread sharedInstance] backgroundInitMain:block];
}
...
+ (void)backgroundInitSub:(dispatch_block_t)block {
  return [[YDThread sharedInstance] backgroundInitSub:block];
}
...
+ (void)backgroundTask:(dispatch_block_t)block {
  return [[YDThread sharedInstance] backgroundTask:block];
}
...
+ (void)backgroundTaskWithDelay:(dispatch_time_t)ms
  ...
  [[YDThread sharedInstance] backgroundTaskWithDelay:ms block:block];
}
